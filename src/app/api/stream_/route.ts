...
async function findStream(embedUrl: string, timeoutMs = 15_000) {
  let chromium: any;
  try {
...
export async function GET(req: NextRequest) {
  ...
  // All sources failed on pass 1 (or deadline hit). If we're using Tor and
  // still have time budget, rotate to a fresh circuit and retry once.
  if (timeLeft() > 15_000) {
    ...
    console.warn("[stream] skipping Tor rotation retry — not enough time budget left");
  }
