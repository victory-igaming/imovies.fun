...
function buildProxyUrl(
  ...
  // Normalize first: decode any existing %XX encoding before re-encoding.
  // Prevents double-encoding (%2B → %252B) when the CDN URL already
  // contains percent-encoded characters like + signs in auth tokens.
  let normalized: string;
  try { normalized = decodeURIComponent(streamUrl); } catch { normalized = streamUrl; }
  const h = encodeURIComponent(JSON.stringify(headers ?? {}));
  let url = `/api/proxy?url=${encodeURIComponent(normalized)}&headers=${h}`;
...
export default function VideoPlayer({
  ...
  // Count consecutive fatal networkErrors before giving up — avoids
  ...
  // NOTE: this effect is intentionally placed AFTER the handleSubSelect
  // useCallback above — registering it before the callback is declared
  // would reference an uninitialised binding (TDZ ReferenceError).
  useEffect(() => {
    ...
    if (isHls) {
      ...
      try {
        ...
        hls.on(Hls.Events.FRAG_LOADED, () => {
          // A fragment loaded successfully — the stream has recovered.
          // Reset the retry counter so a future blip gets the full retry budget.
          networkErrRetriesRef.current = 0;
        });
        ...
        hls.on(Hls.Events.ERROR, (_: any, data: any) => {
          ...
          if (data.type === "networkError") {
            // Segment/manifest/key fetch failed — often a transient proxy
            // hiccup or an expired CDN auth token on the underlying URL.
            // Try HLS.js's built-in resume (startLoad) a few times before
            // falling back — this avoids restarting the whole player (and
            // playback position) for what's frequently a single dropped request.
            if (networkErrRetriesRef.current < MAX_NETWORK_RETRIES) {
              networkErrRetriesRef.current++;
              console.warn(`[VideoPlayer] networkError — retry ${networkErrRetriesRef.current}/${MAX_NETWORK_RETRIES}`);
              try {
                hls.startLoad();
                return;
              } catch { /* fall through to doFallback below */ }
            }
          }
